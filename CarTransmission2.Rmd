---
title: "Regression Models Project 2: Improved version"
...
The mpg of AM=1 (manual transmission) is clearly higher. At a first glance, this is our answer. Manual transmissions have higher mpg (more gas efficiency). But look at weight in Plot 2 in the Appendix. There is a difference in weight by transmission type. The weight of AM=1 (manual) is clearly lower. We might expect weight to be related to mpg and it is (see Plot 3). Therefore weight is a confounder (a variable related both to the independent and dependent variables).

We asked a subject matter expert (the person with the most speeding tickets) to choose one other factor that might be a confounder. He suggested horsepower. It does indeed seem that horsepower is related to transmission type (Plot 4).

Now we do a simple (or marginal) fit of mpg to transmission type.

```{r simplefit, echo=FALSE}
...
`r rownames(mtcars[which.max(hatvalues(fit1)),])` is the most extreme (positive). The fitted mpg (when fitted without this car) is greater than the actual by the most. `r rownames(mtcars[which.min(hatvalues(fit1)),])` is extreme (negative). The fitted mpg (when fitted without this car) is less than the actual value by the most.
...
## Boxplot of MPG
boxplot(mpg~am,mtcars,main="Miles per gallon by transmission type. Plot 1")
```
